...
module.exports = {
    ...
    async execute(message, bot) {
        ...
        CREATE TABLE IF NOT EXISTS \`ticket\`(
            ...
            bot.db.query(`SELECT * FROM ticket WHERE guildId = "${message.guild.id}"`, (err, req) => {
                if(!req.length) {
                    bot.db.query(`INSERT INTO ticket(guildId, guildOwnerId, guildName) VALUES ("${message.guild.id}", "${message.guild.ownerId}", "${message.guild.name}")`)
                }
            })
        })
    }
